...
class tireListView(ListView):
    model = tire
    template_name='tire_list.html'

def tirecreate(request):
    def dispatch(self, *args, **kwargs):
        return super().dispatch(*args, **kwargs)
    emplist = EmployeeMasterlist.objects.all()
    vlist = VehicleMasterList.objects.all()
    return render(request, 'tire_create.html',{'Title':'Tire - Tire','emplist':emplist,'vlist':vlist})

def tiresubmit(request):
    if request.method == 'POST':
        ...
        return HttpResponseRedirect('/Tire/List')

class tireDelete(BSModalDeleteView):
    model = tire
    template_name = 'tire_delete.html'
    success_message = 'Success: Tire Maintenance was deleted.'
    success_url = reverse_lazy('tire_List')


class tireUpdateView(SuccessMessageMixin, UpdateView):
    model = tire
    form_class = tireform
    template_name = 'tire_update.html'
    
    def get_success_message(self, cleaned_data):
    	print(cleaned_data)
    	return "tire Maintenance Updated Successfully!"

class tireDetailView(DetailView):
    model = tire
    template_name = 'tire_details.html'

def tire_deadline(request):
    def dispatch(self, *args, **kwargs):
        return super().dispatch(*args, **kwargs)
    # dl = datetime.datetime.today() - timedelta(days=3)
    dl = tire.objects.filter(Deadline__date = datetime.datetime.today() + timedelta(days=1))
    dl2 = tire.objects.filter(Deadline__date = datetime.datetime.today() + timedelta(days=2))
    dl3 = tire.objects.filter(Deadline__date = datetime.datetime.today() + timedelta(days=3))
    dl4 = tire.objects.filter(Deadline__date = datetime.datetime.today() + timedelta(days=4))
    dl5 = tire.objects.filter(Deadline__date = datetime.datetime.today() + timedelta(days=5))
    dl6 = tire.objects.filter(Deadline__date = datetime.datetime.today())
    return  render(request, 'tire_deadline.html',{'title':'tire - tire Deadline', 'dl':dl, 'dl2':dl2, 'dl3':dl3, 'dl4':dl4, 'dl5':dl5, 'dl6':dl6})


def tire_excel(request):
    repair_queryset = tire.objects.all()   
    response = HttpResponse(
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    )
    response['Content-Disposition'] = 'attachment; filename=tire Maintenance.xlsx'
    workbook = Workbook()

    worksheet = workbook.active
    worksheet.title = 'tire Maintenance'
...
def tireHistoryView(request):
    if request.method == "GET":
       obj = tire.history.all()

       return render(request, 'tire_historical.html', context={'object': obj})

# Registration Daily Report
def tire_report(request):
    ...
    file_name = "Tire Maitenance.xlsx"
    output['Content-Disposition'] = 'attachment; filename='+ file_name
    wb = Workbook()
    ws = wb.active

    #header
    ws.title = "Tire Maintenance"
    ws['A1'].value = "SUBJECT:"
