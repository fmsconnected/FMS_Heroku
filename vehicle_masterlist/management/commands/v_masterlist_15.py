...
def account_email():
    ...
    if month == 2:
        if sec_week_of_month == given_date:
            ...
            exc = Q(ACQ_DATE__year=year) | Q(ACQ_DATE__year=year1) | Q(ACQ_DATE__year=year2) | Q(smoke="Yes")
            car_status = VehicleMasterList.objects.filter(vehicle_status="Active",confirmation="No",
    ...
    if month == 2:
        if sec_week_of_month == given_date:
            ...
            exc = Q(ACQ_DATE__year=year) | Q(ACQ_DATE__year=year1) | Q(ACQ_DATE__year=year2) | Q(confirmation="Yes")
            car_status = VehicleMasterList.objects.filter(vehicle_status="Active",smoke="No",
                PLATE_ENDING="3", Status="Yes").exclude(exc)[:80]
            print(car_status)
            plate = ""
            for carreg in car_status:
