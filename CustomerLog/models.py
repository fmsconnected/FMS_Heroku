...
def increment_Activity_id():
    ...
    if not last_in:
        return 'CS' + str(datetime.datetime.today().strftime('%Y')) + '-' + '000001'
    in_id = last_in.Activity_id
    in_int = int(in_id[10:])
    new_in_int = in_int + 1
    new_in_id = 'CS' + str(datetime.datetime.today().strftime('%Y')) + \
...
class CS_log(models.Model):
    ...
    Date_received = models.CharField(max_length=100, blank=True)
    ...
    def get_absolute_url(self):
        return reverse('CS_List', kwargs={'pk': self.pk})
