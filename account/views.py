...
class Lmasterlist(APIView):
    ...
    def get(self, request, format=None):
        ...
        return Response(data)
class fuel_consumption(APIView):
    authentication_classes = []
    permission_classes = []
    ...
    def get(self, request, format=None):
        ...
        fuel_data_overall = [petron_volume_total,petron_overall_total]
        fuel_data_volume = [shell_volume_total,shell_overall_total]
        fuel_data = {
...
class fuel_volume_monthly(APIView):
    ...
    def get(self, request, format=None):
        ...
        print('Month',month)
        jan_shell_amount =Fuel_supplier.objects.filter(SOA_billdate__year=date.year).filter(SOA_billdate__month__gte=month,Fuel_provider__contains="SHELL").aggregate(Sum('SOA_current_amount'))['SOA_current_amount__sum']
        jan_shell_volume = Fuel_supplier.objects.filter(SOA_billdate__year=date.year).filter(SOA_billdate__month__gte=month,Fuel_provider__contains="SHELL").aggregate(Sum('liters'))['liters__sum']
        
        jan_petron_amount =Fuel_supplier.objects.filter(SOA_billdate__year=date.year).filter(SOA_billdate__month__gte=month,Fuel_provider__contains="PETRON").aggregate(Sum('SOA_current_amount'))['SOA_current_amount__sum']
        jan_petron_volume = Fuel_supplier.objects.filter(SOA_billdate__year=date.year).filter(SOA_billdate__month__gte=month,Fuel_provider__contains="PETRON").aggregate(Sum('liters'))['liters__sum']
